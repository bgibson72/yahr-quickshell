...
Rectangle {
            ...
            Rectangle {
                ...
                MouseArea {
                    ...
                    onClicked: {
                        ...
                        root.dndStateChanged(root.dndEnabled)
                        if (root.dndEnabled) {
    ...
    // Process to get notification list from mako history (includes dismissed notifications)
    Process {
        ...
        stdout: SplitParser {
            onRead: data => {
                ...
                console.log("Notification history chunk:", data)
            }
        }
        
        onRunningChanged: {
            console.log("notificationListProcess running changed:", running, "buffer length:", buffer.length)
            if (!running && buffer !== "") {
                // Parse text-based history output
                const lines = buffer.trim().split('\n')
                const notifs = []
                let currentNotif = null
                
                console.log("Parsing history buffer, lines:", lines.length)
                
                for (let line of lines) {
                    // Match "Notification XX: Title"
                    const notifMatch = line.match(/^Notification (\d+): (.+)$/)
                    if (notifMatch) {
                        if (currentNotif) {
                            notifs.push(currentNotif)
                        }
                        currentNotif = {
                            id: parseInt(notifMatch[1]),
                            summary: notifMatch[2],
                            body: "",
                            appName: "",
                            urgency: "normal",
                            appIcon: "󰂚"
                        }
                    } else if (currentNotif) {
                        // Parse metadata lines
                        if (line.includes("App name:")) {
                            currentNotif.appName = line.replace(/^\s*App name:\s*/, '').trim()
                        } else if (line.includes("Urgency:")) {
                            const urgencyStr = line.replace(/^\s*Urgency:\s*/, '').trim()
                            currentNotif.urgency = urgencyStr
                        }
                    }
                }
                ...
                console.log("Loaded", root.notifications.length, "notifications from history")
                if (notifs.length > 0) {
                    console.log("First notification:", JSON.stringify(notifs[0]))
                }
