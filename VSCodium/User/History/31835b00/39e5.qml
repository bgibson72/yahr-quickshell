                ...
                try {
                    ...
                    for (let line of lines) {
                        ...
                        if (notifMatch) {
                            ...
                            currentNotif = {
                                id: { data: parseInt(notifMatch[1]) },
                                summary: { data: notifMatch[2] },
                                body: { data: "" },
                                "app-name": { data: "" },
                                urgency: { data: 1 }
                            }
                            ...
                            if (line.includes("App name:")) {
                                currentNotif["app-name"].data = line.replace(/^\s*App name:\s*/, '')
                            } else if (line.includes("Urgency:")) {
                                const urgencyStr = line.replace(/^\s*Urgency:\s*/, '')
                                currentNotif.urgency.data = urgencyStr === "critical" ? 2 : (urgencyStr === "low" ? 0 : 1)
                            }
                        }
                    }
