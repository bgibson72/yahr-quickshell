...
Rectangle {
    ...
    property bool dndEnabled: NotificationService.dndEnabled
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: data => {
                ...
                console.log("Notification list chunk:", data)
            }
        }
        ...
        onRunningChanged: {
            ...
            if (!running && buffer !== "") {
                // Parse JSON output from makoctl list
                const response = JSON.parse(buffer)
                const notifs = response.data?.[0] || []
                
                // Convert to our format
                root.notifications = notifs.map(n => ({
                    id: n.id?.data || 0,
                    summary: n.summary?.data || "",
                    body: n.body?.data || "",
                    appName: n["app-name"]?.data || "Notification",
                    urgency: n.urgency?.data === 2 ? "critical" : (n.urgency?.data === 0 ? "low" : "normal"),
                    appIcon: "󰂚"
                }))
                
                console.log("Loaded", root.notifications.length, "active notifications")
                if (root.notifications.length > 0) {
                    console.log("First notification:", JSON.stringify(root.notifications[0]))
                }
