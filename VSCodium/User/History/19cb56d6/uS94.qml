        ...
        stdout: SplitParser {
            onRead: data => {
                try {
                    ...
                    if (settings.general && settings.general.clockFormat24hr !== undefined) {
                ...
                if (use24HourFormat) {
                    digitalTime.text = `${hours.toString().padStart(2, '0')}:${minutes}`
                } else {
        ...
        stdout: SplitParser {
            onRead: data => {
                try {
                    const settings = JSON.parse(data)
                    let location = ""
                    if (settings.general && settings.general.weatherLatitude && settings.general.weatherLongitude) {
                    ...
                    weatherProcess.command = ["sh", "-c", `curl -s 'wttr.in/${location}?format=%c|%t|%C|%h|%w'`]
                    weatherProcess.running = true
                } catch (e) {
                    console.error("Failed to parse settings, using auto-detect:", e)
                    weatherProcess.command = ["sh", "-c", "curl -s 'wttr.in/?format=%c|%t|%C|%h|%w'"]
                    weatherProcess.running = true
                }
            }
        }
