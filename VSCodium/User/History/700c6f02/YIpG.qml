...
Rectangle {
    ...
    Column {
        ...
        Row {
            ...
            // Weather Section - Square shape (approximately 360x360)
            Rectangle {
                width: 280
            ...
            // Calendar Section
            Rectangle {
                width: parent.width - 280 - 12  // Fill remaining space (total width - weather width - spacing)
                height: parent.height
                color: ThemeManager.surface1
                radius: 16
        ...
        stdout: SplitParser {
            onRead: data => {
                ...
                if (parts.length >= 5) {
                    ...
                    // Remove leading plus sign from temperature
                    let temp = parts[1] || "N/A"
                    temperatureText.text = temp.replace(/^\+/, "")
