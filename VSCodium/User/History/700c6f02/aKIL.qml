...
Rectangle {
                    ...
                    onRead: data => {
                        try {
                            ...
                            if (settings.general) {
                                ...
                                console.log("Time settings - 24hr:", use24Hour, "showSeconds:", showSecs)
                            }
                            ...
                            console.log("Time updated to:", timeText.text, periodText.text)
                        } catch (e) {
    ...
    Process {
        ...
        stdout: SplitParser {
            onRead: data => {
                try {
                    console.log("=== Settings Loader - Raw data received ===")
                    console.log(data)
                    const settings = JSON.parse(data)
                    console.log("=== Settings parsed successfully ===")
                    console.log("Full settings object:", JSON.stringify(settings))
                    ...
                    if (settings.general) {
                        ...
                        console.log("useFahrenheit value:", settings.general.useFahrenheit)
                    }
                    
                    const weatherCmd = `curl -s 'wttr.in/${location}?${unit}&format=%c|%t|%C|%h|%w'`
