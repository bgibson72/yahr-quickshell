...
if [ "$DELAY" != "0" ] && [ -n "$DELAY" ]; then
    ...
    systemd-run --user --scope sh -c "sleep $DELAY && $CMD" >> /tmp/quickshell-screenshot.log 2>&1
else
    echo "[Screenshot] Executing: $CMD" >> /tmp/quickshell-screenshot.log
    systemd-run --user --scope sh -c "$CMD" >> /tmp/quickshell-screenshot.log 2>&1
fi

echo "[Screenshot] Process launched via systemd-run" >> /tmp/quickshell-screenshot.log
exit 0
