...
QtObject {
    ...
    // Process components for reading files
    property var themeReadProcess: null
    property var fileReadProcess: null
    
    // Component function to parse theme file
    function loadTheme() {
        let homeDir = Quickshell.env("HOME")
        
        // For now, parse the theme file directly
        // We'll read tokyonight-night as default
        parseThemeFile()
    }
    ...
    function parseThemeFile() {
        ...
        console.log("Loading theme from:", themeFile)
        
        // Create a process to read the file
        if (!fileReadProcess) {
            let comp = Qt.createComponent("Quickshell/Io", "Process")
            if (comp.status === Component.Ready) {
                fileReadProcess = comp.createObject(themeManager, {
                    command: ["cat", themeFile],
                    running: true
                })
                
                fileReadProcess.exited.connect(() => {
                    let content = fileReadProcess.stdout
                    parseThemeContent(content)
                    fileReadProcess.destroy()
                    fileReadProcess = null
                })
            }
        }
    }
