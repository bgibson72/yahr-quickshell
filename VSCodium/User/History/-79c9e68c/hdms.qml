...
ShellRoot {
    ...
    // Notification service - must be first to initialize before other components
    NotificationService {
        id: notificationServiceInternal
    ...
    // Expose notificationService as a property so children can access it via shellRoot.notificationService
    property var notificationService: notificationServiceInternal
    
    Component.onCompleted: {
        console.log("✅ ShellRoot completed, notificationService property is:", notificationService)
    }
    
    property bool calendarVisible: false
    property bool notificationCenterVisible: false
    property bool dndEnabled: notificationServiceInternal.dndEnabled
    ...
    // Notification Popups - stacked on right edge
    Variants {
        model: Quickshell.screens
        ...
        PanelWindow {
            ...
            property var activePopups: shellRoot.notificationService.notifications.filter(n => {
                if (!n.timestamp) {
                    console.log("⏰ Notification missing timestamp:", n.id)
                    return false
                }
                const age = Date.now() - n.timestamp
                const shouldShow = age < 10000  // Show for 10 seconds max
                if (!shouldShow) {
                    console.log("⏰ Notification too old:", n.id, "age:", age)
                }
                return shouldShow
            }).slice(0, 5)
            
            property int calculatedHeight: activePopups.length > 0 ? (activePopups.length * 170) : 100
            
            onCalculatedHeightChanged: {
                console.log("📏 calculatedHeight changed to:", calculatedHeight, "for", activePopups.length, "popups")
            }
            
            onActivePopupsChanged: {
                console.log("💬 Active popups changed. Count:", activePopups.length)
                console.log("📐 Calculated height should be:", calculatedHeight)
                console.log("🪟 Window visible:", visible)
                console.log("📏 Window implicitHeight:", implicitHeight)
                console.log("📏 Container height:", popupContainer.implicitHeight)
            }
            
            // Only show popups if there are active popups AND DND is disabled
            visible: activePopups.length > 0 && !shellRoot.dndEnabled
            
            onVisibleChanged: {
                console.log("👁️ Popup window visibility changed to:", visible, "(DND:", shellRoot.dndEnabled, ")")
            }
            ...
            // Calculate height based on number of popups
            // Average popup height ~150px + 12px spacing
            implicitWidth: 420
            ...
            Column {
                ...
                // Popups will be dynamically added here via Repeater
                Repeater {
                    id: popupRepeater
                    model: popupWindow.activePopups
                    
                    onCountChanged: {
                        console.log("🔁 Repeater count changed to:", count, "model length:", model ? model.length : 0)
                    }
                    ...
                    delegate: NotificationPopup {
                        ...
                        width: 400
                        notification: popupWindow.activePopups[index]
                        
                        Component.onCompleted: {
                            console.log("🔍 Delegate index:", index, "notification:", notification ? notification.summary : "null")
                        }
                        
                        onCloseRequested: {
                            // Dismiss the notification when popup is closed
                            if (notification && notification.id) {
                                shellRoot.notificationService.dismissNotification(notification.id)
                            }
                        }
                    }
                }
        ...
        PanelWindow {
            ...
            Process {
                ...
                onExited: {
                    if (exitCode === 0) {
                        try {
                            let settings = JSON.parse(stdout)
                            console.log("🎨 Bar settings loaded:", JSON.stringify(settings.bar))
                            if (settings.bar && settings.bar.transparentBackground !== undefined) {
                                transparentBackground = settings.bar.transparentBackground
                                console.log("🎨 Bar transparentBackground set to:", transparentBackground)
                            }
                        } catch (e) {
            ...
            implicitHeight: 42
            color: transparentBackground ? "transparent" : ThemeManager.bgBaseAlpha
