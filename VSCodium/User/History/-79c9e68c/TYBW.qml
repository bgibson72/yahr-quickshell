    ...
    Variants {
        ...
        PanelWindow {
            ...
            property var activePopups: shellRoot.notificationService.notifications.filter(n => {
                ...
                return age < 10000  // Show for 10 seconds max
            }).slice(0, 5)
            ...
            Column {
                ...
                Repeater {
                    model: shellRoot.notificationService.notifications.filter(n => {
                        // Show popup for new notifications (less than 10 seconds old)
                        if (n.closed) return false
                        if (!n.timestamp) return false
                        const age = Date.now() - n.timestamp
                        return age < 10000  // Show for 10 seconds max
                    }).slice(0, 5)  // Max 5 popups at once
