...
Scope {
    ...
    function loadThemes() {
        themes = [];
        const proc = Process.exec("bash", ["-c", "ls ~/.config/hypr/themes/*.conf | xargs -n1 basename | sed 's/.conf$//' | sort"]);
        if (proc.exitCode === 0) {
            const themeList = proc.stdout.trim().split('\n').filter(t => t.length > 0);
            themes = themeList;
            
            // Load current theme
            const currentProc = Process.exec("cat", ["~/.config/hypr/.current-theme"]);
    ...
    function applyTheme(themeName) {
        ...
        Process.exec("bash", ["-c", `. ~/.config/quickshell/theme-switcher-quickshell 2>/dev/null; apply_theme "$HOME/.config/hypr/themes/${themeName}.conf" "${themeName}"`]);
    }
