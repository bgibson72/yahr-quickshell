...
impl SimpleComponent for App {
    ...
    fn update(&mut self, msg: Self::Input, sender: ComponentSender<Self>) {
        self.update_internal(msg, sender.clone());
    }
}

impl App {
    fn update_internal(&mut self, msg: AppMsg, sender: ComponentSender<Self>) {
        match msg {
            AppMsg::OpenImage => {
                ...
                if let Some(ref path) = self.current_image {
                    ...
                    match ColorExtractor::extract_from_path(path, self.color_count) {
                        Ok(colors) => {
                            ...
                            while let Some(child) = self.color_grid.first_child() {
                                self.color_grid.remove(&child);
                            }
                            
                            for color in &self.colors {
                                let card = create_color_card(color, &sender);
                                self.color_grid.append(&card);
                            }
                        }
                        Err(e) => {
