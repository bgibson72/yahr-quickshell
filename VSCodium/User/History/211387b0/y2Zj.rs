...
fn create_placeholder_image() -> gtk::gdk::Texture {
    // Create a simple placeholder image (200x200 with a gradient or pattern)
    use gtk::gdk_pixbuf::Pixbuf;
    
    // Create a 200x200 pixbuf with a nice gradient
    let width = 200;
    let height = 200;
    let pixbuf = Pixbuf::new(gtk::gdk_pixbuf::Colorspace::Rgb, false, 8, width, height)
    ...
    unsafe {
        ...
        for y in 0..height {
            for x in 0..width {
                let idx = (y as usize * rowstride) + (x as usize * n_channels);
                ...
                pixels[idx] = (100.0 + factor_x * 80.0) as u8;      // R
                pixels[idx + 1] = (120.0 + factor_y * 100.0) as u8; // G
                pixels[idx + 2] = (180.0 + factor_x * 50.0) as u8;  // B
            }
        }
    }
...
fn create_placeholder_color_card() -> gtk::Box {
    let card = gtk::Box::builder()
        .orientation(Orientation::Vertical)
        .spacing(8)
        .css_classes(vec!["color-card", "placeholder"])
        .build();
    
    card.set_margin_all(8);

    // Placeholder color preview (empty gray box)
    let preview = gtk::Box::builder()
        .orientation(Orientation::Vertical)
        .width_request(140)
        .height_request(140)
        .css_classes(vec!["color-preview", "placeholder-preview"])
        .build();
    ...
    let hex_label = gtk::Label::builder()
        .label("------")
        .selectable(false)
        .css_classes(vec!["color-hex", "placeholder-text"])
        .build();
    card.append(&hex_label);
    ...
    let rgb_label = gtk::Label::builder()
        .label("---")
        .selectable(false)
        .css_classes(vec!["color-rgb", "placeholder-text"])
        .build();
    card.append(&rgb_label);

    // Empty button box (no buttons for placeholders)
    let button_box = gtk::Box::builder()
        .orientation(Orientation::Horizontal)
        .spacing(4)
        .height_request(32)
