...
# fall back to settings.json, then hyprland.conf's source line.
...
if [ -z "$theme_name" ] || [ ! -f "$THEME_FILE" ]; then
    # Try settings.json as second fallback
    if [ -f "$SETTINGS_JSON" ] && command -v python3 >/dev/null 2>&1; then
        theme_name=$(python3 -c "import json,sys; d=json.load(open(sys.argv[1])); print(d.get('theme',{}).get('current',''))" "$SETTINGS_JSON" 2>/dev/null)
        THEME_FILE="$HYPR_THEMES_DIR/${theme_name}.conf"
    fi
fi

if [ -z "$theme_name" ] || [ ! -f "$THEME_FILE" ]; then
    # Final fallback: parse hyprland.conf — expand ~ to $HOME so the path resolves
    THEME_FILE=$(grep "^source.*themes.*\.conf" "$HYPRLAND_CONF" | sed 's/.*= *//' | sed "s|~|$HOME|g")
