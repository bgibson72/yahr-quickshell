...
# VS Code Theme Sync Script
# Reads colors from Hyprland theme and applies them to VS Code (Microsoft version)
...
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo -e "${BLUE}  VS Code Theme Sync${NC}"
echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
echo ""
...
# Map theme colors to VS Code color customizations
# Background colors
...
# Create/update VS Code color customizations using Python
python3 << EOF
import json
import os
...
# Preserve existing font settings if they exist
if "editor.fontFamily" not in settings:
    settings["editor.fontFamily"] = "'Maple Mono NF', 'MesloLGS NF', 'Symbols Nerd Font', monospace"
if "editor.fontSize" not in settings:
    settings["editor.fontSize"] = 14
if "terminal.integrated.fontFamily" not in settings:
    settings["terminal.integrated.fontFamily"] = "'Maple Mono NF', 'MesloLGS NF', 'Symbols Nerd Font'"
if "editor.fontLigatures" not in settings:
    settings["editor.fontLigatures"] = True
if "terminal.integrated.gpuAcceleration" not in settings:
...
if [ $? -eq 0 ]; then
    echo -e "${GREEN}✓ VS Code colors synchronized with $THEME_NAME theme${NC}"
    echo ""
    echo "Restart VS Code to see the changes!"
else
    echo -e "${YELLOW}⚠ Failed to update VS Code settings${NC}"
    echo "Your backup is at: $VSCODE_SETTINGS.backup"
    exit 1
fi
